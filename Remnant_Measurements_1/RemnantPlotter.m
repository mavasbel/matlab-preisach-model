...
for voltage=100:100:1600
    ...
    for i=1:length(measurements)
        ...
        if strcmp(measurements(i).sampleName, fullName)>0 && ...
            ...
            measurements(i).time(end)<=5 && ...
            length(measurements(i).time)==401
            ...
            idx = find(measurements(i).time>=2.75 & measurements(i).time<=3.0, ...
                1, 'first');
            remnant.voltageOffset = remnant.voltageOffset + measurements(i).voltage(idx);
            remnant.offsetStrain = remnant.offsetStrain + measurements(i).strain(idx);
            
            % Find and add final strain
            idx = find(measurements(i).time>=4.25 & measurements(i).time<=4.5, ...
                1, 'first');
            remnant.finalStrain = remnant.finalStrain + measurements(i).strain(idx);
