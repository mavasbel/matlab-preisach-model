...
for voltage=100:100:1600
    ...
    for i=1:length(measurements)
        ...
        if strcmp(measurements(i).sampleName, fullName)>0 && ...
            ...
            idx = find(measurements(i).time>=9.375 & measurements(i).time<=9.725);
            ...
            idx = find(measurements(i).time>=10.75 & measurements(i).time<=11.0);
            remnant.finalStrain = remnant.finalStrain + mean(measurements(i).strain(idx));
            
            % Find and add max strain during initialization
            idx = find(measurements(i).time>=0.0 & measurements(i).time<=8.0);
