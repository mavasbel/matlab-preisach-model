...
% pulse1TimeLimits = [3.0, 3.5];
% afterPulse1TimeLimits = [4.25, 4.5];
% pulse2TimeLimits = [4.5, 5.0];
% afterPulse2TimeLimits = [5.75, 6.0];

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Name prefix, suffix, and time limits for trapezoidal inputs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% namePrefix = 'RMN_1400V_500V_';
% nameSuffix = 'V';
% initTimeLimits = [0, 2.0];
% afterInitTimeLimits = [2.75, 3.0];
...
for voltage=200:100:1600
    ...
    for i=1:length(measurements)
        ...
        if strcmp(measurements(i).sampleName, fullName)>0 && ...
            ...
            % Find and add max voltage and strain during pulse 2
            idx = find(measurements(i).time>=pulse2TimeLimits(1) & measurements(i).time<=pulse2TimeLimits(2));
            remnant.pulse2MaxVoltage = [remnant.pulse2MaxVoltage, max(measurements(i).voltage(idx))];
            remnant.pulse2MaxStrain = [remnant.pulse2MaxStrain, max(measurements(i).strain(idx))];
            
            % Find and add strain after pulse 2
            idx = find(measurements(i).time>=afterPulse2TimeLimits(1) & measurements(i).time<=afterPulse2TimeLimits(2));
            remnant.afterPulse2Strain = [remnant.afterPulse2Strain, mean(measurements(i).strain(idx))];
...
% Plot remnant change vs pulse 2 amplitude
figure; hold on; grid on;
plot(averages.pulse2MaxVoltage - averages.afterInitVoltage, ...
    averages.afterPulse2Strain - averages.afterPulse1Strain,...
    '-o', ...
    'MarkerSize', 5)
for i=1:length(remnants) % Add each measurement independent
    for j=1:length(remnants{i}.pulse1MaxVoltage)
        plot(remnants{i}.pulse2MaxVoltage(j) - remnants{i}.afterInitVoltage(j), ...
            remnants{i}.afterPulse2Strain(j) - remnants{i}.afterPulse1Strain(j), ...
            'o', ...
            'Color', zeros(1,3), ...
            'MarkerSize', 4)
    end
